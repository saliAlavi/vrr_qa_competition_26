...
from vrrqa.utils.prompts import SYSTEM_PROMPT, build_question_prompt, parse_choice  # noqa
...
def main():
    ...
    samples = ds._samples[: args.limit] if args.limit else ds._samples
    ...
    def do_one(sample):
        ...
        pool_f = respdir / f"{sample.question_id}__t{args.temperature}.json"
        texts = []
        if pool_f.exists():
            texts = json.load(open(pool_f)).get("texts", [])
        else:  # seed from any legacy per-k cache file at the same temperature
        ...
        if need > 0:
            ...
            for i in range(need):
                ...
                cfg = types.GenerateContentConfig(temperature=temp, max_output_tokens=args.max_tokens)
                for attempt in range(6):
                    try:
                        resp = client.models.generate_content(model=args.model, contents=[f, prompt], config=cfg)
                        break
    ...
    out = config.SUBMISSIONS_DIR / f"{args.tag}__{args.split}.json"
